function [Pmap,S] = calcPmapGM(T1map,normM0map,GMM)
...
G = GMM.NumComponents; % get number of tissue components from GMModel
...
%pK = GMM.ComponentProportion; % Get pK (component proportion) from GMM
pK = GMM.phi;
pT1 = zeros(size(T1map)); % intialize pT1 matrix
pT1GivenK = zeros([X Y Z N G]); % intialize pT1GivenK matrix
pKGivenT1 = zeros([X Y Z N G]); % intialize pKGivenT1 matrix

pT1(T1map>0) = pdf(GMM,T1map(T1map>0)); % Calculate pT1  of all T1 values in T1map
T1mask = cat(5,T1map>0,false(X,Y,Z,N,G-1)); % Initialize mask to be used for calculation of each PT1GivenK, masking the first position in the 5th dimension (the dimension of K)
% Calculate pT1GivenK and pKGivenT1
for K = 1:G
    pT1GivenK(T1mask) = pdf('normal',T1map(T1map>0),GMM.mu(K),sqrt(GMM.Sigma(K))); % Calculate pT1GivenK for the K position in the 5th dimension
...
Pmap = squeeze(sum(repmat(normM0map,[1 1 1 1 G]) .* pKGivenT1,4));
