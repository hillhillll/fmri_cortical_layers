function [T1,M0,normM0,SS,NumOfComponents,PredictedM,MSE] = calcT1(data,TIlist,Nmax,varargin)
...
for ind=1:2:length(varargin);
    switch(lower(varargin{ind}))
        ...
        case 'gmmean'
            GMmean = varargin{ind+1};
        case 'gmstd'
            GMstd = varargin{ind+1};
        case 'wmmean'
            WMmean = varargin{ind+1};
        case 'wmstd'
            WMstd = varargin{ind+1};
        case 'csfmean'
            CSFmean = varargin{ind+1};
        case 'csfstd'
            CSFstd = varargin{ind+1};
        otherwise
...
h = optimset('Display',Display,'Diagnostics',Diagnostics,'DiffMaxChange',DiffMaxChange,'DiffMinChange',DiffMinChange,...
    'TolFun',TolFun,'TolX',TolX);
...
SS_vec = zeros(1,Nmax); % Sum of squares of optimization
...
NGmax = Nmax-2; % Initial number of components
...
% The number of components is optimized by using a chi-squared
    ...
    NG = N-2;
    %     x0 = [linspace(700,2000,N) (mIR/N)*ones(1,N)]; % inital conditions. x0(1:N) is T1 values, x0(N+1:end) is M0 values
    %     lb = zeros(1,2*N); % lower bounds
    %     ub = 4000*ones(1,2*N); % upper bounds
%     x0 = [600 3000 linspace(900,1800,NG) (mIR.*ones(1,N)./N)]; % inital conditions. x0(1:N) is T1 values, x0(N+1:end) is M0 values
%     lb = [500 1900 (800*ones(1,NG)) zeros(1,N)]; % lower bounds
%     ub = [800 4000 (2000*ones(1,NG)) (1.5*mIR*ones(1,N)) ]; % upper bounds
    x0 = [WMmean CSFmean linspace(GMmean-1.5*GMstd,1.5*GMmean+GMstd,NG) (mIR.*ones(1,N)./N)]; % inital conditions. x0(1:N) is T1 values, x0(N+1:end) is M0 values
    lb = [(WMmean-2*WMstd) (CSFmean-2*CSFstd) ((GMmean-2*GMstd)*ones(1,NG)) zeros(1,N)]; % lower bounds
    ub = [(WMmean+2*WMstd) (CSFmean+2*CSFstd) ((GMmean+2*GMstd)*ones(1,NG)) (1.15*mIR*ones(1,N))]; % upper bounds
    ...
    %[x,SS_vec(N),residual_matrix(N,:),exitflag,output,L,J] = lsqnonlin(@optT1,x0,lb,ub,h,data,TIlist,N);
    [x,SS,~,exitflag,output,L,J] = lsqnonlin(@optT1,x0,lb,ub,h,data,TIlist,N);
    
    T1 = x(1:N); % get T1 values from x
    M0 = x(N+1:end); % get M0 values from x
    NumOfComponents = N;
    %residual = residual_matrix(minMSE,:);
    normM0 = M0 ./ sum(M0); % normalized values of M0
    % calculate expected signal for each component:
    M_c = (repmat(M0',1,K) .* ...
        (1-2*exp(-repmat(TIlist,Nmax,1)./repmat(T1',1,K))) .* ...
        (1-exp(-repmat(TR,Nmax,K)./repmat(T1_mat(N,:)',1,K))));
    % sum signal across all components:
        PredictedM = abs(sum(M_c,1));
    ...
    df = K - 2*N;
    MSE = SS./df;
    LogL = K.*log(1./sqrt(2.*pi.*MSE))-SS./(2*MSE^2);
    BIC = log(K)*N-2*LogL;
...
function residuals = optT1(x,data,TIlist,N)
...
% calculate expected signal for each component:
M_c = (repmat(M0',1,length(TIlist)) .* ...
    (1-2*exp(-repmat(TIlist,length(T1),1)./repmat(T1',1,length(TIlist)))) .* ...
    (1-exp(-repmat(TR,length(T1),length(TIlist))./repmat(T1',1,length(TIlist)))));
% sum signal across all components:
M = abs(sum(M_c,1));

% calculate residuals:
residuals = M-data;
% regularization = 0.1*M0;
% inverse_fusion = 0.01*(4000 - abs(T1(1:end-1)-T1(2:end)));
% % reg = [M 0.1*M0 T1(1:end-1)];
% % data_reg = [data zeros(size(M0)) T1(2:end)];
% % residuals = M_reg-data_reg;
% % Err = sum((M-data).^2);
% % Err_reg = sum(residuals.^2);
% % Err_reg;
% residuals = [residuals regularization inverse_fusion];
end
