function [varargout] = plotTMModel(T1Histogram,TMModel,varargin)
...
k = TMModel.NumComponents;
mu = TMModel.mu;
sigma = sqrt(TMModel.Sigma);
phi = TMModel.phi;
nu = TMModel.nu;
[mu,I] = sort(mu);
sigma = sigma(I);
phi = phi(I);
hold on
plot(0:4000,pdf(TMModel,0:4000),'k','LineWidth',2);
ColorScheme = hsv(k);
% ColorScheme = ColorScheme(1:k,:);
for j=1:k
    x(j,1:500) = linspace(mu(j)-4*sigma(j),mu(j)+4*sigma(j),500);
    y(j,1:500) = phi(j).*pdf('tlocationscale',x(j,1:500),mu(j),sigma(j),nu(j));
end
