function [T1,M0,normM0,residual,SS,NumOfComponents,PredictedM,MSE] = calcT1(data,TIlist,Nmax,varargin)
...
while N<=Nmax
    ...
    df(N) = K - 2*N;
    MSE(N) = SS_vec(N)./df(N);
    LogL(N) = K.*log(1./sqrt(2.*pi.*MSE(N)))-SS_vec(N)./(2*MSE(N)^2);
...
T1 = T1_mat(minMSE,:);
M0 = M0_mat(minMSE,:);
NumOfComponents = minMSE;
residual = residual_matrix(minMSE,:);
SS = SS_vec(minMSE);
PredictedM = PredictedM(minMSE,:);
...
M_c = (repmat(M0',1,length(TIlist)).*(1-2*exp(-repmat(TIlist,length(T1),1)./repmat(T1',1,length(TIlist)))) ...
        .* (1-exp(-TR./repmat(TIlist,length(T1),1))));
% sum signal across all components:
M = abs(sum(M_c,1));
